...
def k_fold_cv(x, y, degree, k=5):
    ...
    phi=poly_features(shu[:,0],degree)
    yy=shu[:,1]
    mss=0
    for i in range(N//k):
        phi_test=phi[k*i:(k*i+k-1),:]
        phi_train=np.concatenate(phi[0:k*i,:],phi[(k*i+k-1):,:])
        y_test=yy[k*i:(k*i+k-1)]
        y_train=np.concatenate(y[0:k*i,:],y[(k*i+k-1):,:])
